...
// world-atlas TopoJSON keys countries by ISO 3166-1 numeric id; full
// numeric → alpha-2 table so any country the backend scores can be shaded.
const NUMERIC_TO_ISO2: Record<string, string> = {
  ...
  "276": "DE", "288": "GH", "300": "GR", "304": "GL", "308": "GD",
...
interface Props {
  riskByIso2: Record<string, CountryRisk>;
  onSelect: (country: CountryRisk | null) => void;
}

export default function WorldMap({ riskByIso2, onSelect }: Props) {
  return (
    <ComposableMap projectionConfig={{ scale: 147 }}>
      <Geographies geography={GEO_URL}>
        {({ geographies }) =>
          geographies.map((geo) => {
            const iso2 = NUMERIC_TO_ISO2[String(geo.id).padStart(3, "0")];
            const country = iso2 ? riskByIso2[iso2] : undefined;
            return (
              <Geography
                key={geo.rsmKey}
                geography={geo}
                fill={country ? sentimentColor(country.score) : "#1a1b1f"}
                stroke="#26282e"
                strokeWidth={0.5}
                onClick={() => onSelect(country ?? null)}
                style={{
                  default: { outline: "none" },
                  hover: {
                    fill: country ? sentimentColor(country.score + 0.1) : "#26282e",
                    outline: "none",
